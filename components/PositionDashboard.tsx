...
import { Shield, TrendingUp, AlertCircle, Activity, ShieldCheck, ChevronRight, BarChart3 } from 'lucide-react'
              ...
              >
                <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex items-center space-x-4">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-semibold text-lg shadow-inner ${
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        <span className="text-base font-semibold text-black tracking-tight">{position.currency} Protection</span>
                        <span className="text-[9px] font-semibold bg-black/5 text-slate-500 px-2 py-0.5 rounded-full tracking-wide">
                          ID: {position.id}
                        </span>
                      </div>
                      <div className="flex items-center mt-1">
                        <ShieldCheck className="w-3 h-3 text-green-600 mr-1.5" />
                        <span className="text-[10px] font-semibold text-green-600 tracking-wide">Active & Protected</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-8 flex-1 md:ml-12">
                    <div>
                      <div className="text-[9px] font-medium text-slate-400 tracking-wide mb-1.5">Collateral</div>
                      <div className="text-xs font-semibold text-black tracking-tight">{position.collateralAmount.toLocaleString()} RLO</div>
                    </div>
                    <div>
                      <div className="text-[9px] font-medium text-slate-400 tracking-wide mb-1.5">Entry rate</div>
                      <div className="text-xs font-semibold text-black">1 {position.currency} = ${position.entryRate.toFixed(4)}</div>
                    </div>
                    <div>
                      <div className="text-[9px] font-medium text-slate-400 tracking-wide mb-1.5">Liability</div>
                      <div className="text-xs font-semibold text-black tracking-tight">${position.borrowedAmount.toLocaleString()}</div>
                    </div>
                    <div>
                      <div className="text-[9px] font-medium text-slate-400 tracking-wide mb-1.5">Health factor</div>
                      <div className={`text-xs font-semibold flex items-center ${isSafe ? 'text-green-600' : 'text-red-600'}`}>
                  ...
                  <button className="h-10 w-10 md:h-12 md:w-12 bg-black/5 group-hover:bg-black group-hover:text-[#E6E4D5] rounded-xl flex items-center justify-center transition-all shadow-sm">
